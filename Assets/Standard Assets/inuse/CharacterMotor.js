...
// setup palyer link
//private var playerFinder : GameObject;
...
var useFixedUpdate : boolean = false;

var weatherAffects : boolean = true;
...
//		finder = 	playerFinder.gameObject.GetComponent(LocationFinder);
}
...
class CharacterMotorMovement {
	...
	var maxForwardSpeed : float = 2;
	var maxSidewaysSpeed : float = 1;
	var maxBackwardsSpeed : float = 0.2;
	
	// Curve for multiplying speed based on slope (negative = downwards)
	var slopeSpeedMultiplier : AnimationCurve = AnimationCurve(Keyframe(0, 1), Keyframe(90, 0));
	
	// How fast does the character change speeds?  Higher is faster.
	var maxGroundAcceleration : float = 0.5;
	var maxAirAcceleration : float = 0.5;
...
class CharacterMotorSliding {
	...
	var slidingSpeed : float = 1;
	
	// How much can the player control the sliding direction?
	// If the value is 0.5 the player can slide sideways with half the speed of the downwards sliding speed.
	var sidewaysControl : float = 0.3;
	
	// How much can the player influence the sliding speed?
	// If the value is 0.5 the player can speed the sliding up to 150% or slow it down to 50%.
	var speedControl : float = 0.0;
}
...
private function UpdateFunction () {
	...
	if (MoveWithPlatform()) {
		...
		// *************************************************************************************kiihtyvyys - sään vaikutus

	// ADD WEATHER to velocity
	if (weatherAffects) {
	//	velocity += (finder.currentWeather * weatherResistance * Time.deltaTime);
	}
...
private function ApplyInputVelocityChange (velocity : Vector3) {	
	...
	var maxVelocityChange : float = GetMaxAcceleration(grounded) * Time.deltaTime *shipAgility;
...
private function GetDesiredHorizontalVelocity () {
	// Find desired velocity
	var desiredLocalDirection : Vector3 = tr.InverseTransformDirection(inputMoveDirection /*+ (finder.currentWeather * weatherResistance * Time.deltaTime)*/ );
	var maxSpeed : float = MaxSpeedInDirection(desiredLocalDirection /*+ (finder.currentWeather * weatherResistance * 0.1) */);	//	flippaa yli jotenkin kovassa warpissa >2.2			// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	// ADD WEATHER
	//if (weatherAffects) desiredLocalDirection += finder.currentWeather * weatherResistance;
...
function GetMaxAcceleration (grounded : boolean) : float {
	...
	if (grounded)
		return movement.maxGroundAcceleration * (shipSpeed + shipAgility + shipAgility /3);
	else
		return movement.maxAirAcceleration * (shipSpeed + shipAgility + shipAgility /3);
}
...
function MaxSpeedInDirection (desiredMovementDirection : Vector3) : float {
	if (desiredMovementDirection == Vector3.zero)
		...
		// *************************************************************************************MAX speed + säävektori * resistanssi

		// ADD WEATHER
		if (weatherAffects) {
			// temp is the ship vector
			temp = new Vector3(temp.x, 0, temp.z * zAxisEllipseMultiplier) * movement.maxSidewaysSpeed;
			// translate weathervector (world) to ship transform (local)
			//var weatherVector : Vector3 = transform.InverseTransformDirection(finder.currentWeather * weatherResistance);
			// add weather drag to max speed
			//temp += weatherVector;
		}
		
		// *************************************************************************************
		
		var length : float = temp.magnitude;
		//var length : float = new Vector3(temp.x, 0, temp.z * zAxisEllipseMultiplier).magnitude * movement.maxSidewaysSpeed;
		
		return length * shipSpeed;
	}
}
...
@script RequireComponent (PlatformInputController)  // CharacterController
