...
public class WeatherDebugGizmos : MonoBehaviour {
    ...
    void drawGizmos () 
    {
        if (on)
        {
            ...
            for (int i=0; i< ParticleFlowData.flowTexture.width;++i)
                for (int j=0; j< ParticleFlowData.flowTexture.height;++j)
            {
                //Debug.Log ("i: "+i+", j: "+j);
                if (i % Nth == 0 && j % Nth == 0)
                {
                    //ps.flowMap[i, j] = new Vector2(flowTexture.GetPixel(i, j).r - 0.5f, ps.flowTexture.GetPixel(i,j).g * (-1.0f) + 0.5f);
                    v = ParticleFlowData.getFlowMap()[i,j];
    ...
    {
        return p*1100.0f / ParticleFlowData.flowTexture.width -550.0f;
    }
    private float toMapZ(int p)
    {
        return p*575.0f / ParticleFlowData.flowTexture.height -287.5f;
    }
